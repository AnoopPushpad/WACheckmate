...
def is_number_available(driver, number, csv_writer):
    ...
    if '<span dir="ltr" aria-label="" class="_ao3e" style="min-height: 0px;">Messages are end-to-end encrypted. No one outside of this chat, not even WhatsApp, can read or listen to them. Click to learn more</span>' in checkingpagesrc:
...
def generate_captcha():
    ...
    while True:
        ...
        user_input = int(input(f"Please enter the sum {num1} + {num2} to proceed: "))
        
        # Check if the user's input is correct
        if user_input == captcha_sum:
            print("CAPTCHA successfully solved! WhatsApp number verification process initiated.")
            break
        else:
...
print("WACheckmate is a bulk WhatsApp number checking tool crafted with Python and utilizing Selenium.\n")
print("1. Configure Parameters and Initiate Automation:")
print("- Adjust the WAIT_TIME parameter to match your internet speed (default is 30 seconds).")
...
print("- After logging into WhatsApp, solve the basic captcha through the terminal when prompted.")

print("\n3. Validate Numbers with Country Code:")
print("- Ensure that numbers in the numbers.txt file include the country code format, like +918516853332 (where +91 represents the country code).")
print("- The script will then extract and validate each number listed in the numbers.txt file.")

print("\n4. Results:")
print("- After validation, the results will be saved in a CSV file within the folder.\n")
...
driver.quit()
print(f"WhatsApp number checking completed! The results have been saved into {csvfilename}.csv")